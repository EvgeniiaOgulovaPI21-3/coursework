...
public class DrinkService {
    ...
    public List<Drink> listAll(String keyword) {
        if (keyword != null && !keyword.isEmpty()) {
            try {
                // Проверяем, является ли ввод числом (поиск по цене)
                Integer price = Integer.parseInt(keyword);
                return drinkRepository.findByPriceLessThanEqual(price);
            } catch (NumberFormatException e) {
                // Если не число, ищем по названию
                return drinkRepository.search(keyword.toLowerCase());
            }
        }
        return drinkRepository.findAll();
    }
